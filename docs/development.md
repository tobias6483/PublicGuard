...
23. Disable every trigger, quit and relaunch PublicGuard, and confirm the trigger list stays fully disabled until a trigger is re-enabled or a preset is applied.
24. Toggle `Settings > Notifications` and confirm the checkmark moves.
25. Toggle `Settings > Lock Screen` and confirm the checkmark moves.
26. In `dist/PublicGuard.app`, toggle `Settings > Launch at Login` and confirm the checkmark moves.
27. Confirm PublicGuard appears in macOS `System Settings > General > Login Items`.
28. Toggle `Settings > Launch at Login` off and confirm it is removed from login items.
29. Click `Arm`.
30. Open `Recent Trigger Status` and confirm Bluetooth armed baseline shows `not seen yet` until the learned target is observed after arming.
31. Disconnect the power adapter.
32. Confirm brief reconnects inside the debounce window do not write `charger_disconnected`.
33. Confirm a sustained disconnect writes `charger_disconnected` to the log.
34. Open `Recent Trigger Status` and confirm the power row now shows the adapter disconnected.
35. With `Settings > Trigger Cooldown` set to 30 seconds or more, trigger a second enabled condition during the cooldown and confirm it is logged as `trigger_ignored name="<trigger>.cooldown"`.
36. Open `Recent Trigger Status` and confirm the trigger cooldown row shows the remaining cooldown or ready state.
37. Set `Settings > Trigger Grace Overrides > Wi-Fi Change` to `30 seconds`, change Wi-Fi networks, and confirm the `grace_period_started` entry uses `seconds=30`.
38. Change Wi-Fi networks, for example from a normal network to a mobile hotspot, and confirm `network_changed kind="ssidChanged"` is logged when that trigger is enabled.
39. Disconnect Wi-Fi and confirm `network_changed kind="disconnected"` is logged when `Settings > Ignore Wi-Fi Disconnects` is off.
40. Turn `Settings > Ignore Wi-Fi Disconnects` on, disconnect Wi-Fi again, and confirm the event is logged as `trigger_ignored name="networkChange.disconnect"`.
41. Open `Recent Trigger Status` and confirm the Wi-Fi row reflects the current SSID or `Unknown / disconnected`.
42. Disable a trigger and confirm the matching event is logged as `trigger_ignored`.
...
The lock screen action is treated as successful only when a configured lock
command exits with status `0`; otherwise PublicGuard falls back to the next
available command or records `lock_screen_failed`.
